...
using Surveys.Services;
    ...
    public class SurveyContentController : ControllerBase
    {
        ...
        private readonly ISurveyContentService _surveyContentService;

        public SurveyContentController(ILogger<SurveyContentController> logger, ISurveyContentService surveyContentService)
        {
            _logger = logger;
            _surveyContentService = surveyContentService;
        }
        ...
        /// <summary>
        /// The goal of this API is to return the specified survey definition
        /// to the caller.
        /// </summary>
        /// <param name="surveyDefinitionId">A unique identifier for a
        /// survey definition.</param>
        /// <returns>A survey definition or</returns>

        [HttpGet]
        [Route("/api/survey/definition/{surveyDefinitionId}")]
        ...
        public IActionResult GetSurveyDefinition(Guid surveyDefinitionId)
        {
            var surveyDefinition = _surveyContentService.RetrieveSurveyDefinition(surveyDefinitionId);

            if (surveyDefinition != null)
            {
                return Ok(surveyDefinition);
            } else
            {
                return NotFound();
            }
